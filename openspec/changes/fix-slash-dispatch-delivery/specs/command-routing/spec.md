...
3. **Path D (fallthrough)**: if neither Path B nor Path C is reachable (i.e. `pi.dispatchCommand` absent AND non-headless session — tmux / wt / unrecognized spawn), the handler SHALL return `false` and let the caller fall through to `pi.sendUserMessage(text, { deliverAs })`. Pi 0.74+ handles extension commands internally via its `prompt()` method → `_tryExecuteExtensionCommand()`, so the command dispatches natively. The bridge SHALL emit a `console.warn` diagnostic noting that `dispatchCommand` is unavailable and the command falls through to `sendUserMessage`. The bridge SHALL NOT emit `command_feedback` events for this path — pi handles execution silently.

The `delivery` parameter (defaulting to `"followUp"` when absent) SHALL determine `streamingBehavior` on the Path B `dispatchCommand` call and `deliverAs` on the Path D `sendUserMessage` fallback.
...
- IS a registered extension command but neither Path B nor Path C is available (Path D returns `false` — pi dispatches natively via `sendUserMessage`), OR
- IS a skill command (`/skill:<name>`), prompt template, or unrecognized slash text whose semantics require LLM interpretation.

The handler SHALL emit EXACTLY ONE `started` event and EXACTLY ONE terminal event (`completed` xor `error`) per dispatch invocation across Path B and Path C combined. Path D emits no `command_feedback` events — the fallthrough and pi-native dispatch are silent.
...
#### Scenario: Path D — non-headless session, dispatchCommand absent (fallthrough)
...
- **THEN** the bridge SHALL emit a `console.warn` diagnostic noting dispatchCommand is unavailable and the command falls through to sendUserMessage
- **AND** the bridge SHALL return `false` from the dispatch helper
- **AND** the caller SHALL call `pi.sendUserMessage("/ctx-stats", { deliverAs })` where `deliverAs` defaults to `"followUp"`
- **AND** pi SHALL dispatch the extension command internally via `_tryExecuteExtensionCommand`
- **AND** the bridge SHALL NOT emit `command_feedback` events for this path
- **AND** the bridge SHALL NOT emit `dispatch_extension_command`

#### Scenario: Path D delivery — steering message uses deliverAs "steer" on sendUserMessage
- **WHEN** `send_prompt` text is `/ctx-stats` AND `msg.delivery === "steer"` AND `pi.dispatchCommand` is NOT a function AND session is non-headless
- **THEN** the bridge SHALL return `false` from the dispatch helper
- **AND** the caller SHALL call `pi.sendUserMessage("/ctx-stats", { deliverAs: "steer" })`

#### Scenario: Path C fallthrough when connection is absent
- **WHEN** `send_prompt` text is `/ctx-stats` AND `pi.dispatchCommand` is NOT a function AND `isHeadlessRpcSession()` returns true AND no `connection` is supplied
- **THEN** the bridge SHALL return `false` from the dispatch helper (Path D fallthrough)
- **AND** the bridge SHALL NOT emit `command_feedback` events

#### Scenario: No command_feedback on Path D fallthrough
- **WHEN** a single dispatch takes Path D
- **THEN** the recorded `command_feedback` events for that command-text SHALL be empty (zero events emitted by the bridge)
...
9. Check for `/` prefix matching a known **extension command** (`source: "extension"` in `pi.getCommands()`, excluding `DASHBOARD_NATIVE_COMMANDS`) → dispatch via `pi.dispatchCommand` (Path B, when available) OR headless RPC dispatch (Path C, when headless) OR fall through to `pi.sendUserMessage` (Path D, non-headless without dispatchCommand — pi dispatches natively)
...
#### Scenario: Extension dispatch in non-headless falls through to pi native dispatch
- **WHEN** `send_prompt` text is `/ctx-stats` AND `pi.dispatchCommand` is NOT a function AND session is non-headless (tmux/wt)
- **THEN** step 9's Path D SHALL return `false`
- **AND** step 10's fall-through to `pi.sendUserMessage("/ctx-stats")` SHALL execute
- **AND** pi SHALL dispatch the command internally via `_tryExecuteExtensionCommand`
...
- **THEN** the bridge SHALL use `"followUp"` semantics (streamingBehavior on dispatchCommand, deliverAs on sendUserMessage)

#### Scenario: delivery "steer" propagates to streamingBehavior
...
#### Scenario: delivery "steer" propagates to deliverAs on fallthrough
- **WHEN** `send_prompt` message includes `delivery: "steer"` AND `pi.dispatchCommand` is NOT available AND session is non-headless
- **THEN** the bridge SHALL call `pi.sendUserMessage(text, { deliverAs: "steer" })`
